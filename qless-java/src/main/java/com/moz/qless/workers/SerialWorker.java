...
public class SerialWorker {
  ...
  private Iterator<Queue> queueItor;
  ...
  public SerialWorker(
    ...
    Preconditions.checkArgument(
        intervalInSeconds > 0,
        "only positive vlaue for intervalInSecods allowed");
    ...
    for (final String queueName : queueNameList) {
      this.queues.add(this.client.getQueues().get(queueName));
    }
  ...
  private Job getJob() throws IOException {
    ...
    while (!this.shutDown) {
      if (!this.queueItor.hasNext()) {
        this.queueItor = this.queues.iterator();
      }

      final Queue queue = this.queueItor.next();
  ...
  public void run() throws InterruptedException, IOException {
    while (!this.shutDown) {
      ...
      if (null == job) {
        LOGGER.info("empty job list and sleep "
